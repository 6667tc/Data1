...
class GNN_MLP(nn.Module):
    def __init__(self, seq_len,input_size,output_size,hid_feats,gat_head):
        ...
        self.gat = GAT(in_feats=self.seq_len, h_feats=64, out_feats=self.seq_len, num_head=self.gat_head)

        self.lstms=nn.ModuleList()
        for k in range(self.input_size):
            self.lstms.append(nn.LSTM(input_size=self.seq_len, hidden_size=64,num_layers=1, batch_first=True, dropout=0.5))


        self.linears=nn.ModuleList()
        for k in range(self.input_size):
            self.linears.append(nn.Sequential(nn.Linear(64,32),nn.ReLU(),nn.Linear(32,self.output_size),nn.Sigmoid()))
    ...
    def forward(self, data):
        ...
        # output(13, 512, 1) y(512, 13, 1)
        x = x.permute(0, 2, 1)
        preds_lstm=[[] for k in range(x.shape[2])]
        for k in range(x.shape[2]):
            out_lstm,_ = self.lstms[k](x[:,:,k])
            out_lstm = self.linears[k](out_lstm)
            preds_lstm[k].append(out_lstm)


        preds_lstm = [torch.stack(x, dim=0) for x in preds_lstm]
        preds_lstm = torch.stack(preds_lstm, dim=0)
        pred = preds_lstm.reshape(self.input_size,32,self.seq_len)
